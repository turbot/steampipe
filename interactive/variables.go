...
func PromptForMissingVariables(ctx context.Context, missingVariables []*modconfig.Variable) error {
	traceCtx, span := instrument.StartSpan(ctx, "PromptForMissingVariables")
	defer span.End()

	fmt.Println()
	fmt.Println("Variables defined with no value set.")
	for _, v := range missingVariables {
		span.AddEvent(fmt.Sprintf("prompt:%s", v.ShortName))
		r, err := promptForVariable(traceCtx, v.ShortName, v.Description)
		if err != nil {
			return err
		}
