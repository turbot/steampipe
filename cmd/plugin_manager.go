...
import (
	...
	sdklogging "github.com/turbot/steampipe-plugin-sdk/v6/logging"
...
func runPluginManagerCmd(cmd *cobra.Command, _ []string) {
	...
	defer func() {
		...
		if err != nil {
			...
			fmt.Println(fmt.Sprintf("%s%s", plugin.PluginStartupFailureMessage, err.Error())) //nolint:forbidigo // acceptable
		}
		os.Exit(1)
	}()
