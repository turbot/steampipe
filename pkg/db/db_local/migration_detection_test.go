...
// | 6  | Migration failed (pre-flight/restore/validation) | full cluster, PG_VERSION  | cleared on retry          | no commit; warn; retry next start                     | TestCrossMajorMigration outcome cases; TestDeletionGate_Public_* /           |
// |    |                                            |                                |                           |                                                       | _DataTank_* preserve tests; TestDeletionGate_WiredStartup_FailurePreservesOldDir |
// | 7  | Data-tank restore partial (>=1 partition failed) | full cluster, PG_VERSION | cleared on retry          | treated as failure, no commit                         | TestDeletionGate_DataTank_PartialSuccess_PreservesEverything; DT-F4, LE-10   |
