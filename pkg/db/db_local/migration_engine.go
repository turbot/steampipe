...
//
// The old side's PG_VERSION is the single decision input for all of the above: present = migration pending (the new
// side is a disposable draft); absent = nothing to migrate. State table:
//
...
//   | dir parked by operator      | explicit opt-out (see backstop below)    | fresh start, no migration    |
//
// BACKSTOP - the migration-incomplete flag (deliberately NOT part of the flow above): alongside the dump, a flag
// file (db/migration-incomplete.flag) is written once a cross-major migration starts writing into the new side, and
// cleared at commit. It exists for exactly one state the PG_VERSION check cannot see: the old data directory being
// UNREACHABLE (not merely absent) while a half-written new-side cluster exists. The new side is a valid, bootable
// cluster from the moment initdb runs, so without the flag that pod would boot the half-restored database and
// silently serve partial data. In a hosted deployment the old data lives behind a mount that is configured per
// render - so the unreachable state arises from ordinary transition mistakes: a renderer working from stale config,
// a fleet-wide config flip racing per-workspace migration completion, a parked workspace waking after the flip, or
// a hand-built pod spec during an incident. The flag turns all of those from silent partial data into a refused
// startup with instructions (prepareDb's guard in install.go; deleting the flag is the operator override). On a
// single machine the old directory cannot "unmount", so the flag is inert there. It is a backstop only: it is never
// consulted to DECIDE a migration, only to refuse startup when no migration is detectable AND the flag says one
// never finished.
