...
//
// This suite is the TEST CONTRACT for the data-tank cross-major migration path
// (exec-5b). It is written against the DESIRED behaviour described in
// data-tank-storage-patterns.md (exec-5):
//
//   - Each data tank is a <handle> + <handle>-parts schema PAIR. The migration
//     must dump/restore every data-tank schema pair, preserving the declarative
//     PARTITION BY LIST(_cloud_partition) topology and re-attaching every
//     partition.
//   - The light-migration variant applies: no collation pre-flight blocking,
//     no row-level checksum gate. The ONLY catalog risk is the SYSTEM_USER
//     reserved word (F05), which must route the migration to tier 3 rather than
//     blocking it.
//   - The restore is TIERED. Tier 1 (parallel pg_restore) is the normal path;
//     tiers 2-5 escalate as each prior tier fails. The outcome names the tier
//     reached, since the operational cost differs per tier (tier 4 = degraded
//     service). When every tier fails the terminal outcome is
//     dtOutcomeDataPreservedOnDisk: under the 2026-06-08 governing decision the
//     new Postgres version still runs, but the old data directory and the safety
//     dump are both kept on disk untouched so nothing is lost (no version-revert).
//
// BASELINE EXPECTATION (this commit, exec-5a):
// ---------------------------------------------
// There is NO data-tank migration code yet. runDataTankMigration below drives a
// minimal self-contained REFERENCE migration (per-data-tank-schema pg_dump +
// pg_restore) so the suite COMPILES and RUNS, but the reference path has no
// tier concept, no reserved-word routing, no per-partition degraded restore,
// and breaks on the hyphenated -parts schema and the reserved-word column. So
// every case FAILS its assertion against this baseline. Those failures ARE the
// spec for exec-5b; exec-5b replaces the reference driver with the real tiered
// data-tank migration and the matrix goes green.
//
// HOW TO RUN
// ----------
...
// cluster + a real PG18 target cluster over Unix sockets, applies fixture SQL,
// runs pg_dump / pg_restore). Policy decisions belong to exec-5b's code.
...
// -----------------------------------------------------------------------------
// Outcome enum. Separate type / separate prefix from exec-2a's migrationOutcome
// because the data-tank value set names the restore TIER reached. (Spec:
// exec-5a task file, "Outcome enum" section.)
// -----------------------------------------------------------------------------

type dataTankMigrationOutcome int

const (
	// Success outcomes - name the tier reached, since the cost of each tier
	// matters operationally (tier 4 is degraded service).
	...
	// dtOutcomePartialRestoreDataPreserved is NOT a success: the per-partition
	// COPY tier (tier 4) climbed and most rows landed, but at least one partition
	// could not be migrated, so that partition's rows never reached the new
	// cluster. Under the 2026-06-08 governing decision a partial result preserves
	// the original on disk (old PG14 data dir + safety dump) and is reported as a
	// failure outcome - it is NOT a success, because the deletion gate must not
	// fire. The engine signals this via errDataTankPartialRestore with
	// committed=false and oldClusterRetained=true.
	...
	// dtOutcomeDataPreservedOnDisk is the terminal failure outcome under the
	// 2026-06-08 governing decision (data-preservation over version-revert): the
	// migration did NOT complete, the new Postgres version still runs (possibly
	// empty/partial), and the original data is preserved on disk in two
	// independent forms - the untouched old PG14 data directory plus the retained
	// safety dump - so it is recoverable. This REPLACES the former
	// dtOutcomeRolledBackToPG14 ("stay on PG14"), which encoded the version-revert
	// policy the decision supersedes. The engine signals this via
	// errDataTankAllTiersFailed with oldClusterRetained=true; the harness asserts
	// the old dir + dump are still on disk.
...
// -----------------------------------------------------------------------------
// Binary locations. Mirrors the exec-2a layout under a data-tank-specific root
// so the two suites don't share a worker tree.
// -----------------------------------------------------------------------------
...
// dtVolumeMultiplier scales the programmatic DT-B volume fixtures down for CI /
// dev runs without editing the case table. Default 1 = full size. Set
// STEAMPIPE_DT_XMIG_VOLUME=0.01 to run DT-B at 1% for a smoke pass.
...
// dtScalePartitions scales partition COUNTS by the same volume knob, but only
// for counts above a small floor - so a smoke run (STEAMPIPE_DT_XMIG_VOLUME
// small) does not spend minutes building tens of thousands of partition tables
// just to load a fixture. At the default multiplier (1.0) the production-scale
// counts (DT-B5 = 600, DT-B6 = 33,600) are preserved exactly, which is what
// feeds exec-5b's ATTACH-cost budget.
...
func dtInitCluster(ctx context.Context, version, dataDir string) error {
	...
	// Mirror production: force libc 'C' locale so the cluster default collation
	// matches what Steampipe ships (install.go initDatabase). PG18's default
	// ICU/builtin collation otherwise diverges.
...
func dtStartCluster(ctx context.Context, version, dataDir, sockDir string) (*dtCluster, error) {
	...
	cmd := exec.CommandContext(ctx, postgres,
		...
		// DT-B6 attaches ~33k partitions; the default max_locks_per_transaction
		// (64) overflows the lock table when a single transaction touches that
		// many relations. Raise it so the fixture can load. (exec-5b's real
...
// dtListDataTankSchemas returns the data-tank schema names present on the source
// cluster: every <handle> schema and its <handle>-parts sibling. This is the
// enumeration the real migration must drive over (instead of just "public").
...
//
// This is NOT the data-tank migration. It is a deliberately minimal stand-in so
// the suite compiles and runs against a branch with no exec-5b code. It dumps
// each discovered data-tank schema with `pg_dump --schema=<name>` and restores
...
//   - no -parts dependency ordering handling (the hyphenated parts schema's
//     partition tables LIKE the parent in the sibling schema, which a naive
//     per-schema dump/restore does not order correctly).
//
// exec-5b replaces this function with the real tiered migration entrypoint and
// the matrix turns green.
// -----------------------------------------------------------------------------

// dtMigrationReport carries WHAT the migration actually did, beyond the final
// outcome enum. The reserved-word / tier-escalation / recovery cases assert on
// this so that an accidental match of the outcome enum (e.g. the reference
// driver returning DataPreservedOnDisk because its plain restore happened to fail)
// does NOT count as a pass. exec-5b populates these fields from the real tiered
// migration; the baseline reference leaves them zero/false.
type dtMigrationReport struct {
	// highestTierAttempted is the last restore tier the migration tried (1..4).
	// Zero means no tiered restore logic ran at all (the baseline reference).
	highestTierAttempted int
	// reservedWordRouted is true if a reserved-word column was detected and the
	// migration routed straight to tier 3. The reserved-word cases assert this.
	reservedWordRouted bool
	// oldClusterRetained is true if, on a failure, the migration left the old PG14
	// data directory in place on disk (the data-preservation guarantee, NOT a
	// version-revert: the new version still runs). DT-E3 / DT-E4 / DT-F5 assert this.
	oldClusterRetained bool
}

// runDataTankMigration is the SEAM exec-5b implements. It adapts the test
// harness's dtCluster handles + dtCaseSetup injection flags onto the real tiered
// data-tank migration engine (migration_datatank.go) and maps the engine's
// result back onto the suite's outcome enum + report.
func runDataTankMigration(ctx context.Context, oldC, newC *dtCluster, backupDir string, setup dtCaseSetup) (dataTankMigrationOutcome, dtMigrationReport, error) {
	...
	if err != nil {
		...
		case errors.Is(err, errDataTankPartialRestore):
			// PARTIAL: the per-partition COPY tier climbed and most rows landed,
			// but ≥1 partition could not be migrated. committed=false, so the
			// deletion gate preserves the old PG14 data dir alongside the dump.
	...
	// No error => the engine fully committed (partition failures already routed to
	// errDataTankPartialRestore above). Name the tier reached.
	...
	case dtRestoreTier4PerPartition:
		// Tier 4 with zero partition failures landed every partition - a full
		// success at the per-partition COPY tier.
		return dtOutcomeAutoRestoreSucceededAtTier3, report, nil
	default:
		return dtOutcomeDataPreservedOnDisk, report, nil
	}
...
// dtClusterRef adapts a test-harness dtCluster into the engine's
// pgClusterRef.
...
// -----------------------------------------------------------------------------
// Volume fixture generation (DT-B). Generated programmatically, never shipped as
// multi-MB .sql files (acceptance criterion: volume fixtures use programmatic
// row generation).
// -----------------------------------------------------------------------------

// dtGenVolumeSQL builds a single-tank data-tank fixture with `partitions`
// partitions and `rowsPerPart` rows in each (both pre-scaled by the volume
// multiplier). Tank handle "vol_aws", table "v".
...
// dtGenVolumeSQLMultiTable builds a multi-table volume fixture: `tables` tables,
// each with `partitionsPerTable` partitions and `rowsPerPart` rows. Used by
// DT-B6 (state-farm workspace aggregate: ~33k partitions across 60 tables).
...
// dtCaseSetup carries per-case harness instructions for the failure-injection
// cases (DT-E operational, DT-F tier escalation, DT-D refresh-mid-flight).
type dtCaseSetup struct {
	...
	// DT-F tier injection: force the named tier(s) to fail so the next tier
	// must pick up. The data-tank migration (exec-5b) reads these to simulate
	// each tier's failure deterministically; the reference driver ignores them
	// (which is why DT-F2..F5 fail on baseline).
...
type dtCase struct {
	...
	// test report (output/data-tank-load-test-<date>.md) is built from measured
...
func dtCases() []dtCase {
	...
	cases := []dtCase{
		...
		// DT-B6: state-farm workspace aggregate (~33k partitions across 60 tables).
		// 60 tables x 560 partitions ~= 33,600 partitions. Small rows-per-part so
		// the ATTACH cost - not the row volume - is what is exercised.
		...
		// DT-D2: pause hook NOT respected => the orchestration contract must
		// prevent this; the desired outcome is the refresh-pause failure, surfaced
		// rather than silently corrupting the dump.
		...
		// DT-D3: migration starts while a partition swap is in flight. Desired:
		// the dump captures a consistent (post-swap-or-pre-swap) snapshot and the
		// restore succeeds at tier 1 (ATTACH transactions are atomic per-table).
		...
		// DT-D4: _mgr_ intermediate-table cleanup in flight. Desired: exec-5b
		// detects _mgr_ infix tables and migrates cleanly (skip-or-wait path).
		...
		// DT-F4: tiers 1-3 fail, tier 4 (per-partition COPY) climbs but one corrupt
		// partition never migrates - a PARTIAL result, NOT a success. committed=false,
		// the old PG14 data dir + safety dump are preserved (the unmigrated partition
		// survives in the original), the deletion gate does not fire.
		...
		// DT-F6: reserved-word fixture re-run; assert the TIER reached (3), via
		// the reserved-word escalation path (not the F3 path).
		...
		//
		// Data tank's schema surface is deliberately narrow: ordinary partitioned
		// tables (PARTITION BY LIST(_cloud_partition)) with bigint/text/jsonb
		// columns and a single primary-key index. It has NO functions, views,
		// rules, extensions, procedural languages, expression/partial/GIN/GiST
		// indexes, or public-schema GRANTs. So most catalogue items simply CANNOT
		// appear in a data-tank schema. The catalogue items that DO map to the data
		// shape get a real fixture; every other item is non-applicable and is
		// covered by the DT-P-NonApplicable-* control (a plain tank that must
		// clean-migrate, proving the engine is not tripped by something irrelevant).
		//
		// Data-shape-relevant catalogue items:
		//   - SYSTEM_USER reserved word -> DT-C1 / DT-F6 (already above): the one
		//     true cross-major risk for data tank; routes to tier 3.
		//   - P18.3 unlogged partitioned table -> DT-P18_3: a data tank could be
		//     created UNLOGGED on PG14; pg_restore of the unlogged DDL fails on PG18,
		//     so the tiered engine must escalate to the per-table COPY tier (tier 3),
		//     which recreates the parent LOGGED and lands the data. No fault flags:
		//     the catalogue trigger itself forces the tier-1/2 failure.
		//   - P18.4 \. -in-CSV -> DT-P18_4: a text value containing "\." round-trips
		//     because the engine streams partition data with binary COPY; tier 1.
		...
		// Non-applicable control. Stands in for every catalogue item that cannot
		// occur in a data-tank schema: P15.1, P15.2, P15.4, P15.5, P15.6, P16.1,
		// P16.2, P16.3, P16.4, P16.5, P17.1, P17.2, P17.3, P17.4, P17.5, P17.6,
		// P18.1, P18.2, P18.5, P18.6. Each of those requires a function / view /
		// rule / extension / language / expression-index / public-GRANT that data
		// tank never has. A plain tank must clean-migrate at tier 1: the engine is
		// not tripped by the absence of these triggers.
...
// dtLoadCases returns the LE-1..LE-11 load / resilience / tier-escalation cases
// (exec-5c). They reuse the same harness, volume generators and fault-injection
// flags as the DT-* matrix but at production-calibrated scale (state-farm: 600
// partitions in the largest single table, ~33,600 partitions per workspace).
// Every LE case carries loadTest=true so runCase captures wall-clock + disk
// metrics into the per-run metrics file (see dtRecordMetrics).
//
// Scale knobs (LE_* volume): the LE-3-derived cases (LE-3, LE-5..LE-11) all use
// the same 600x60k = 36M-row single-tank fixture so their wall-clocks are
// directly comparable (LE-8 = 1.5xLE-3, LE-9 = 2xLE-3, etc. budgets in the task).
func dtLoadCases() []dtCase {
	...
	return []dtCase{
		...
		// LE-5: LE-3 + concurrent refresh attempt during migration, pause hook honoured.
		// Refresh-pause respected => migration completes at tier 1.
		...
		// LE-10: LE-3 + tier-1+2+3 failure + one corrupt partition. Tier 4 lands the
		// rest, but the one bad partition never migrates - a PARTIAL result, NOT a
		// success: committed=false, the old PG14 data dir + safety dump are preserved
		// (the unmigrated partition survives in the original), the deletion gate does
		// not fire.
...
// runCase executes one case end-to-end and returns the observed outcome plus an
// assertion error (for success cases whose per-schema/per-partition golden
// mismatches between PG14 and PG18).
func (w *dtWorker) runCase(ctx context.Context, tc dtCase) (dataTankMigrationOutcome, error) {
	...
	// Load-test metric capture (exec-5c). Measured AFTER the migration returns so
	// the dump dir + target data dir reflect peak on-disk footprint of the run.
	if tc.loadTest {
	...
	// Report-level assertions: HOW the outcome was reached. These fail against
	// the baseline reference (which leaves the report empty), preventing an
	// accidental enum match from passing.
	...
	// failure-ending outcome the original must remain recoverable on disk - the old
	// PG14 data directory present and populated, plus (where a dump was taken) the
	// safety dump directory retained. Asserted per failure case so a regression
	// that deletes the old dir before the migration is 100% complete is caught here.
	// (exec-6d is the dedicated gate test; this is the per-case guard.)
	if isDataTankFailureOutcome(got) {
		...
		// On a dump failure / disk pre-flight abort / refresh-pause abort the dump
		// directory may be absent by definition (the dump never completed); the old
		// data dir is the surviving copy. When the dump DID run and the restore then
		// failed (DataPreservedOnDisk) or only partially completed
		// (PartialRestoreDataPreserved), the safety dump directory must be retained.
	...
	// For success cases with a golden, run the assert SQL on both clusters and
	// compare digests (per-schema, per-partition topology + row counts).
...
// isDataTankFailureOutcome reports whether an outcome is a terminal failure where
// the data-preservation invariant must hold (old dir preserved; dump retained
// where one was taken).
...
// dtAssertOldDataDirPreserved confirms the source PG14 data directory still exists
// and still holds a real cluster (its PG_VERSION marker present) after a failed
// migration - the data-preservation guarantee that the original is recoverable.
...
// dtAssertDumpDirRetained confirms the directory-format safety dump is still on
// disk and non-empty after an all-tiers-failed outcome (the second independent
// recovery copy required by the governing decision).
...
// -----------------------------------------------------------------------------
// Load-test metric capture (exec-5c). Each LE-* case appends one JSON line to
// the metrics file named by STEAMPIPE_DT_XMIG_METRICS (default: a file under the
// test root). The load-test report is built from these measured numbers.
// -----------------------------------------------------------------------------
...
// dtRecordMetrics appends one metric line. Append-mode + a mutex keeps it safe
// across the parallel subtests.
...
// dtDirSize returns the total on-disk bytes under dir (best-effort; unreadable
// entries are skipped). Used to measure dump-dir + cluster footprint.
...
func TestDataTankMigration(t *testing.T) {
	...
	// Each case runs as its own subtest so the standard `-run` filter selects
	// individual cases (e.g. -run TestDataTankMigration/DT-B5 for the timing
	// run). A buffered semaphore bounds concurrency to dtParallelism(); a
