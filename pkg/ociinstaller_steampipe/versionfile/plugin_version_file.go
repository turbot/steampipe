...
func (f *PluginVersionFile) EnsurePluginVersionFile(installData *InstalledVersion) error {
	pluginFolder, err := filepaths_steampipe.FindPluginFolder(installData.Name)
	if err != nil {
		return err
	}
...
func (f *PluginVersionFile) Save() error {
	...
	versionFilePath := filepaths_steampipe.PluginVersionFilePath()
...
func recomposePluginVersionFile() *PluginVersionFile {
	...
	versionFiles, err := filehelpers.ListFiles(filepaths_steampipe.EnsurePluginDir(), &filehelpers.ListOptions{
