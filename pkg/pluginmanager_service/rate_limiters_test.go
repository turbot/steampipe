...
func TestPluginManager_ConcurrentUpdateRateLimiterStatus(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < iterations; i++ {
			// This simulates line 96 in plugin_manager_rate_limiters.go
			// where userLimiters is written without lock protection
	...
	go func() {
		defer wg.Done()
		for i := 0; i < iterations; i++ {
			// This calls updateRateLimiterStatus which reads from userLimiters
			// at line 161 without lock protection
