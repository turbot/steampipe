...
func (c *InteractiveClient) handleInitResult(ctx context.Context, initResult *db_common.InitResult) {
	...
	c.initialiseSuggestions(ctx) //nolint:golint,errcheck // worst case is we won't have autocomplete - this is not a failure
...
func (c *InteractiveClient) readInitDataStream(ctx context.Context) {
	...
	go c.listenToPgNotifications(listenCtx) //nolint:golint,errcheck // worst case is autocomplete isn't update - not a failure
