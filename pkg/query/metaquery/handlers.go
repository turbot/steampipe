...
func setOrGetSearchPath(ctx context.Context, input *HandlerInput) error {
	if len(input.args()) == 0 {
		...
		display.ShowWrappedTable(
			...
			&display.ShowWrappedTableOptions{Wrap: true, AutoMerge: false},
		)
	} else {
		arg := input.args()[0]
		paths := []string{}
...
`, constants.Bold(".inspect {connection}.{table}"))
		...
		display.ShowWrappedTable(header, rows, &display.ShowWrappedTableOptions{Wrap: true, AutoMerge: true})
	}
...
func listConnections(ctx context.Context, input *HandlerInput) error {
	...
	display.ShowWrappedTable(header, rows, &display.ShowWrappedTableOptions{Wrap: true, AutoMerge: false})
