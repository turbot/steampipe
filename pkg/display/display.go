...
func ShowOutput(ctx context.Context, result *queryresult.Result, resolvedQuery *modconfig.ResolvedQuery, searchPath []string, opts ...DisplayOption) int {
	rowErrors := 0
	...
	case constants.OutputFormatSnapshotShort, constants.OutputFormatSnapshot:
		rowErrors, timingResult = displaySnapshot(ctx, result, resolvedQuery, searchPath)
...
func displaySnapshot(ctx context.Context, result *queryresult.Result, resolvedQuery *modconfig.ResolvedQuery, searchPath []string) (int, *queryresult.TimingResult) {
	...
	snapshotData := map[string]any{
		...
		"Panels":        getPanels(ctx, result, resolvedQuery),
		"SearchPath":    searchPath,
		"StartTime":     "2024-09-11T18:47:09.181169+05:30",
		"EndTime":       "2024-09-11T18:47:09.182591+05:30",
		"Layout":        getLayout(result, resolvedQuery),
	}
	...
	rowErrors := 0
	_, err = iterateResults(result, func(row []interface{}, result *queryresult.Result) {
...
func getPanels(ctx context.Context, result *queryresult.Result, resolvedQuery *modconfig.ResolvedQuery) map[string]PanelData {
...
func getData(ctx context.Context, result *queryresult.Result) map[string]interface{} {
...
func getLayout(result *queryresult.Result, resolvedQuery *modconfig.ResolvedQuery) map[string]any {
	hash, err := utils.Base36Hash(resolvedQuery.RawSQL, 8)
	if err != nil {
		return nil
	}
...
func displayCSV(ctx context.Context, result *queryresult.Result) (int, *queryresult.TimingResult) {
	...
	csvWriter.Comma = []rune(cmdconfig.Viper().GetString(pconstants.ArgSeparator))[0]
