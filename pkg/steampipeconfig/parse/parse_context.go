...
func (r *ParseContext) AddDependencies(block *hcl.Block, name string, dependencies map[string]*modconfig.ResourceDependency) hcl.Diagnostics {
	...
	for _, dep := range dependencies {
		...
		for _, t := range dep.Traversals {
			parsedPropertyPath, err := modconfig.ParseResourcePropertyPath(hclhelpers.TraversalAsString(t), dep.Range)
...
func (r *ParseContext) getDependencyOrder() ([]string, error) {
	...
	for _, d := range rawDeps {
		...
		// TODO COMMENT - passing an empty hcl.Range, since hcl.Range is only used in ParseResourcePropertyPath
		// in case of errors and we are confident an error cannot happen here
		propertyPath, err := modconfig.ParseResourcePropertyPath(d, hcl.Range{})
		if err != nil {
			return nil, err
		}
