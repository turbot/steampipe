...
func installPluginBinary(image *SteampipeImage, tempdir string, pluginDir string) error {
	...
	if _, err := ungzip(sourcePath, pluginDir); err != nil {
		return fmt.Errorf("could not unzip %s to %s", sourcePath, pluginDir)
	}
	return nil
}

func installPluginDocs(image *SteampipeImage, tempdir string, pluginDir string) error {
	...
	destPath := filepath.Join(pluginDir, "docs")
