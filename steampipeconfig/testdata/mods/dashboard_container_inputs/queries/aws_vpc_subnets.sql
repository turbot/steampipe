with analysis as (
    ...
    from aws_region region
             inner join
         aws_vpc vpc
         on region.region = vpc.region
    where vpc.vpc_id = 'vpc-0bf2ca1f6a9319eea'
    union
    select region as parent,
           vpc_id  as id,
           title  as name,
           1      as depth,
           'aws_vpc'  as category
    from aws_vpc
    where vpc_id = 'vpc-0bf2ca1f6a9319eea'
    union
    select sg.vpc_id        as parent,
           group_id         as id,
           group_name         as name,
           2                as depth,
    ...
    where sg.vpc_id = 'vpc-0bf2ca1f6a9319eea'
    union
    select subnet.vpc_id        as parent,
           subnet.subnet_id         as id,
           subnet.title         as name,
           2                as depth,
           'aws_vpc_subnet' as category
    from aws_vpc_subnet subnet
             inner join
         aws_vpc vpc
         on subnet.vpc_id = vpc.vpc_id
    where subnet.vpc_id = 'vpc-0bf2ca1f6a9319eea'
    union
    select
        sg_id as parent,
        fn.arn as id,
