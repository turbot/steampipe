...
func parseModHcl(modPath string, fileData map[string][]byte, opts *LoadModOptions) (*modconfig.Mod, error) {
	...
	for _, block := range content.Blocks {
		...
		case modconfig.BlockTypeQuery:
			...
			query.ReflectionData = getReflectionDataForParsedResource(types.SafeString(query.ShortName), block, fileData)
			queries[name] = query
		...
		case modconfig.BlockTypeControl:
			...
			control.ReflectionData = getReflectionDataForParsedResource(control.Name(), block, fileData)
		...
		case modconfig.BlockTypeControlGroup:
			...
			controlGroup.ReflectionData = getReflectionDataForParsedResource(controlGroup.Name(), block, fileData)
	...
	// NOTE: This updates the 'Mod' field in the reflection data of each resource
	mod.SetQueries(queries)
	mod.SetControls(controls)
	mod.SetControlGroups(controlGroups)
...
func createPseudoResources(modPath string, mod *modconfig.Mod, opts *LoadModOptions) error {
	...
	for _, path := range sourcePaths {
		...
		if resource != nil {
			reflectionData := getReflectionDataForPseudoResource(resource.Name(), mod.Name(), path, fileData)
			resource.SetReflectionData(reflectionData)
...
func getReflectionDataForPseudoResource(name, modName string, path string, fileData []byte) *modconfig.ReflectionData {
	sourceDefinition := string(fileData)
	split := strings.Split(sourceDefinition, "\n")
	lineCount := len(split)
	lastLineLength := len(split[lineCount-1])

	sourceRange := hcl.Range{
		Filename: path,
		Start:    hcl.InitialPos,
		End:      hcl.Pos{lineCount, lastLineLength, 0},
	}
	return &modconfig.ReflectionData{
		ModName:          modName,
		ResourceName:     name,
		Range:            sourceRange,
