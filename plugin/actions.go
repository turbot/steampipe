...
func Remove(image string, pluginConnections map[string][]modconfig.Connection) error {
	...
	installedTo := filepath.Join(file_paths.PluginDir(), filepath.FromSlash(fullPluginName))
...
func List(pluginConnectionMap map[string][]modconfig.Connection) ([]PluginListItem, error) {
	...
	filepath.Walk(file_paths.PluginDir(), func(path string, info os.FileInfo, err error) error {
		if !info.IsDir() && strings.HasSuffix(info.Name(), ".plugin") {
			rel, err := filepath.Rel(file_paths.PluginDir(), filepath.Dir(path))
			if err != nil {
				return err
			}
