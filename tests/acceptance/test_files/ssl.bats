...
@test "expiry year of root.crt should be 9999 and server.crt should be 3yrs from now" {
  ...
  run openssl x509 -enddate -noout -in $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/root.crt
  echo $output
  # check enddate
  ...
  run openssl x509 -enddate -noout -in $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/server.crt
  echo $output
  # check enddate
...
@test "restarting service should not rotate root and server certificates" {
  ...
  # save file hash
  run cksum $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/root.crt
  id_root=$(echo $output | awk '{print $1}')
  echo $id_root

  # save file hash
  run cksum $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/server.crt
...
@test "deleting root certificate, service start should regenerate server and root certs" {
  ...
  rm -f $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/root.crt
...
@test "adding an encrypted private key should work fine and service should start successfully" {
  ...
  run openssl genrsa -aes256 -out $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/server.key -passout pass:steampipe -traditional 2048 
  
  run openssl req -key $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/server.key -passin pass:steampipe -new -x509 -out $STEAMPIPE_INSTALL_DIR/db/18.4.0-rc.1/data/server.crt -subj "/CN=steampipe.io"
