...
@test "steampipe check search_path when passed through command line" {
  cd $FUNCTIONALITY_TEST_MOD
  run steampipe check control.search_path_test_2 --output json --search-path chaos,b,c --export output.json
  assert_equal "$(cat output.json | jq '.controls[0].results[0].status')" '"ok"'
  rm -f output.json
}
...
@test "steampipe check search_path and search_path_prefix when passed through command line" {
  cd $FUNCTIONALITY_TEST_MOD
  run steampipe check control.search_path_test_3 --output json --search-path chaos,b,c --search-path-prefix aws --export output.json
  assert_equal "$(cat output.json | jq '.controls[0].results[0].status')" '"ok"'
  rm -f output.json
}
